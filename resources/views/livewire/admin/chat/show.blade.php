<div>
  <x-header :title="'Chat dengan ' . $conversation->user->name" subtitle="ID Percakapan: {{ $conversation->id }}" separator>
    <x-slot:actions>
      <x-button icon="lucide.arrow-left" class="btn-error btn-sm" link="/admin/chat" />
    </x-slot:actions>
  </x-header>
  ...
  <div class="flex flex-col h-[70vh] bg-base-100 rounded-lg shadow-xl overflow-hidden" x-data="{
    ...
    <div x-ref="messagesDiv" class="flex-grow p-4 overflow-y-auto" wire:poll.{{ $pollingEnabled ? '2s' : 'none' }}="pollMessages">
      ...
      <div @class([ 'chat' , 'chat-end'=> $isSender, // Pesan dari pengirim (kamu) ada di kanan
        'chat-start' => !$isSender, // Pesan dari lawan bicara ada di kiri
        ])>
      ...
      <div class="text-center text-gray-500 py-10">
        ...
        <p class="mt-2 text-lg">
          ...
          Mulai percakapan Anda dengan {{ $conversation->user->name }}.
          @else
          Mulai percakapan Anda dengan admin.
          @endif
        </p>
      </div>
      @endforelse
